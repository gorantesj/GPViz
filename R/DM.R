...
diamonds %>%
  ...
  geom_violin(width=1, alpha = .25, color = "#FFFFFF") +
  geom_boxplot(width=0.1, color="#92ADB0", outlier.alpha =.3) +
...
graficar_distribucion <- function(bd, variable, violin = F, flip = T){
  nn <-  bd %>%  count({{variable}})

  bd %>%
    left_join(nn) %>% group_by(cut) %>%  mutate(m = median({{variable}})) %>%
    mutate(tamano = paste0(cut, "\n", n %>%  comma)) %>%
    ggplot( aes(x=fct_reorder(tamano, -m), y={{variable}}, fill=cut)) +
    geom_violin(width=1, alpha = .25, color = "#FFFFFF") +
    geom_boxplot(width=0.1, color="#92ADB0", outlier.alpha =.3) +
    geom_point(aes(y = m), color = "#FFFFFF", shape = 18)+
    scale_fill_manual(values = c("#0D5359", "#720026", "#CE4257", "#FF7F51", "#BFBD28"))+
